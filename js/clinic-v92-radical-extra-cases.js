(() => {
  ...
  if (!C || C.__v92ExtraCasesLoaded) return;
  C.__v92ExtraCasesLoaded = true;
  ...
  function visibleDate(card){
    /*
     * V92 RADICAL FIX:
     * Do NOT depend on the visual date text.
     * The scheduler already places the exact YYYY-MM-DD on seat buttons.
     * This works whether the header is "22 Aug 2026", "22/08/2026",
     * Arabic, English, or reformatted later.
     */
    const dated = card.querySelector('[data-date]');
    if(dated?.dataset?.date && /^\d{4}-\d{2}-\d{2}$/.test(dated.dataset.date)){
      return dated.dataset.date;
    }

    const startEl = card.querySelector('[data-start]');
    if(startEl?.dataset?.start){
      const fromStart = cairoDateOf(startEl.dataset.start);
    ...
    for(const [name,n] of Object.entries(months)){
      m=lower.match(new RegExp('(\\d{1,2})\\s+'+name+'\\s+(\\d{4})','i'));
  ...
  async function refresh(){
    ...
    try{
      ...
      console.warn('V92 extra cases:',e);
    }finally{
      busy=false;
