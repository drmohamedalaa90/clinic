window.ClinicPages['dashboard'] = async function () {
  ...
  if (C.isDoctor() && !C.hasRole('owner')) {
    const [
      ...
      C.sb
        .from('appointments')
        .select('id,patient_id,status,scheduled_start')
    ...
    cards = [
      ...
      ['☀', C.lang==='ar' ? 'مرضى اليوم' : "Today's Patients",
        new Set(a.map(x => x.patient_id)).size],
    ...
    recent = `
      <section class="content-card">
        <div class="section-head">
          <div>
            <span class="eyebrow">${C.lang==='ar' ? 'اليوم' : 'TODAY'}</span>
            <h3>${C.lang==='ar' ? 'مسار العيادة' : 'Clinic flow'}</h3>
          </div>

          <button class="secondary-button" data-go="queue">
            ${C.lang==='ar' ? 'فتح قائمة الانتظار' : 'Open Queue'}
          </button>
        </div>

        <div class="flow-strip">
          ${['booked','confirmed','arrived','waiting','with_doctor','completed']
            .map(s => `
              <div>
                <strong>${a.filter(x => x.status === s).length}</strong>
    ...
    const queries = [
      C.sb
        .from('appointments')
        .select('id,patient_id,status,doctor_id,scheduled_start')
        .gte('scheduled_start', start)
        .lte('scheduled_start', end),
    ...
    cards = [
      ...
      ['👥', C.lang==='ar' ? 'المرضى اليوم' : 'Patients Today',
        new Set(appts.map(x => x.patient_id)).size],
      ...
      ['💳', C.lang==='ar' ? 'دخل اليوم' : "Today's Income",
        C.formatMoney(income.total_received || 0)],
      ...
      <section class="content-card">
        <div class="section-head">
          <div>
            ...
            <h3>${C.lang==='ar' ? 'حالة العيادة اليوم' : 'Today at a glance'}</h3>
          </div>

          ${C.isReception()
            ? `<button class="secondary-button" data-go="reception">
                 ${C.lang==='ar' ? 'فتح الاستقبال' : 'Open Reception'}
               </button>`
            : ''
          }
        </div>
        ...
        <div class="flow-strip">
          ${['booked','confirmed','arrived','waiting','with_doctor','completed']
            .map(s => `
              <div>
                <strong>${appts.filter(x => x.status === s).length}</strong>
                <span>${s.replaceAll('_',' ')}</span>
              </div>
            `).join('')}
        </div>
  ...
  document.getElementById('mainContent').innerHTML = `
    <section class="dashboard-welcome">
      <div>
        <span class="eyebrow">${C.lang==='ar' ? 'مرحباً' : 'WELCOME'}</span>
        <h2>${C.escape(name)}</h2>
    ...
    <!--
      ...
      Visible on every member's dashboard.
      Content is role-aware, but approved apologies and logistics
      deficiencies are shared with all active members.
    -->
    <section
    ...
    >
      <div class="centered compact-loading">
        ...
        <p class="muted">
          ${C.lang==='ar' ? 'جاري تحميل الإشعارات...' : 'Loading notifications...'}
        </p>
      </div>
    </section>
  ...
  document
    ...
    .forEach(button => {
      button.addEventListener(
        'click',
        () => C.route(button.dataset.go)
      );
    });

  // Fetch once now so the dashboard notification block is immediately live.
  await window.ClinicNotifications?.refresh?.();
};
