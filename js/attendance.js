(function(){
  ...
  const weekdays={en:['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday'],ar:['الاثنين','الثلاثاء','الأربعاء','الخميس','الجمعة','السبت','الأحد']};
  ...
  async function render(){
    ...
    async function showToday(){
      ...
      area.innerHTML=`<div class="attendance-today-grid"><article class="attendance-clock-card"><span class="eyebrow">${today}</span><h3>${sch?`${sch.start_time.slice(0,5)} → ${sch.end_time.slice(0,5)}`:(c.lang==='ar'?'لا يوجد جدول اليوم':'No schedule today')}</h3><div class="attendance-state">${record?`${c.statusPill(record.check_out_at?'completed':'active')}<strong>${c.lang==='ar'?'دخول':'In'}: ${c.formatTime(record.check_in_at)}</strong><strong>${c.lang==='ar'?'خروج':'Out'}: ${c.formatTime(record.check_out_at)}</strong><span>${c.lang==='ar'?'تأخير':'Late'} ${record.late_minutes||0} min • ${c.lang==='ar'?'خروج مبكر':'Early'} ${record.early_leave_minutes||0} min</span>`:`<span class="muted">${c.lang==='ar'?'لم يتم تسجيل الحضور بعد.':'No attendance recorded yet.'}</span>`}</div>${!c.isManagement()&&sch?`<div class="form-actions">${!record?`<button id="staffCheckIn" class="primary-button compact">${c.lang==='ar'?'تسجيل حضور':'Check in'}</button>`:!record.check_out_at?`<button id="staffCheckOut" class="primary-button compact">${c.lang==='ar'?'تسجيل انصراف':'Check out'}</button>`:''}</div>`:''}${c.isManagement()?`<div class="form-actions"><button id="manualAttendance" class="secondary-button">${record?(c.lang==='ar'?'تصحيح السجل':'Adjust record'):(c.lang==='ar'?'إدخال يدوي':'Manual attendance')}</button><button id="addStaffSchedule" class="secondary-button">+ ${c.lang==='ar'?'جدول عمل':'Work schedule'}</button></div>`:''}</article><article><h3>${c.lang==='ar'?'الجدول الأسبوعي':'Weekly schedule'}</h3><div class="stack-list space-top">${(schedules||[]).length?(schedules||[]).map(s=>`<div class="list-card"><div><strong>${weekdays[c.lang][Number(s.weekday)-1]}</strong><div class="small-note">${s.start_time.slice(0,5)} → ${s.end_time.slice(0,5)} • ${c.lang==='ar'?'سماح':'Grace'} ${s.late_grace_minutes} min</div></div>${c.statusPill(s.is_active?'active':'inactive')}</div>`).join(''):`<div class="empty-state">${c.t('noData')}</div>`}</div></article></div>`;
    ...
    async function showLeave(){
      ...
      area.innerHTML=`<div class="section-head"><h3>${c.lang==='ar'?'طلبات الإجازة':'Leave requests'}</h3>${!c.isManagement()?`<button id="newLeave" class="primary-button compact">+ ${c.lang==='ar'?'طلب إجازة':'Leave request'}</button>`:''}</div>${data?.length?`<div class="stack-list">${data.map(x=>`<article class="list-card"><div><div class="list-title">${x.start_date} → ${x.end_date}</div><div class="small-note">${c.escape(x.leave_type.replaceAll('_',' '))}${x.note?` • ${c.escape(x.note)}`:''}</div></div><div class="list-actions">${c.statusPill(x.status)}${c.isManagement()&&x.status==='pending'?`<button class="table-action success-outline" data-leave="${x.id}" data-action="approve">✓</button><button class="table-action danger-outline" data-leave="${x.id}" data-action="reject">✕</button>`:''}</div></article>`).join('')}</div>`:`<div class="empty-state">${c.t('noData')}</div>`}`;
